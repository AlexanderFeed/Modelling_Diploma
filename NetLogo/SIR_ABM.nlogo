...
exposed_;;the number of exposed people
width  ;;the width of the world
time ;;for exposed
]

turtles-own[
status ;1 for susceptible, 2 for infected, 3 for recovered, 4 for exposed, 5 for isolated
]
...
to go
  ...
  ;;exposed turtles get infected
  ask turtles with [status = 4][
    if time = 0 [
      set status 2
     set time 50
    ]
    if time > 0 [set time time - 1]
  ]
...
;;update the number of each type of agents
  ...
  set exposed_ count turtles with [status = 4]
...
;;plot the numbers of susceptibles, infecteds, recovereds over time
  ...
  if plot-pen-exists? "recovereds" [
   ...
   if plot-pen-exists? "exposed" [
   set-current-plot-pen "exposed"
   plotxy ticks exposed_
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
